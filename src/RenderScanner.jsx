...
const TIER_BG    = {
  S: "rgba(240,192,64,0.13)", A: "rgba(74,240,144,0.10)",
  B: "rgba(64,170,255,0.10)", C: "rgba(170,170,170,0.07)"
};
...
function getPHT() {
  return new Date().toLocaleString("en-PH", {
    timeZone:"Asia/Manila", hour:"2-digit",
    minute:"2-digit", second:"2-digit", hour12:true
  });
}
function getTVLink(coin) {
  return `https://www.tradingview.com/chart/?symbol=BINANCE:${coin}.P`;
}
...
export default function RenderScanner() {
  ...
  const [sentSignals, setSentSignals] = useState(new Set());
  ...
  const runScan = useCallback(async () => {
    ...
    // ─── SEND TELEGRAM FOR S AND A TIER ──────────────────────────────────────
    const scanTime = getPHT();
    const topSetups = out.filter(r => (r.tier === "S" || r.tier === "A") && r.entryPattern);

    if (topSetups.length > 0) {
      const newSetups = topSetups.filter(r => {
        const key = `${r.symbol}-${r.tier}-${r.entryPattern}`;
        return !sentSignals.has(key);
      });
...
Found <b>${newSetups.length}</b> new setup(s)!`
        ...
        const newKeys = new Set(sentSignals);
        for (const r of newSetups) {
          const emoji  = r.entryPattern === "RGR" ? "🟢" : "🔴";
          const bias   = r.entryPattern === "RGR" ? "BULLISH ▲" : "BEARISH ▼";
          const tvLink = getTVLink(r.symbol.replace("USDT",""));
          const msg =
`🎯 <b>VALID FVG SETUP</b>
━━━━━━━━━━━━━━━
${emoji} <b>${r.symbol.replace("USDT","")}/USDT</b> — ${bias}
...
━━━━━━━━━━━━━━━`;
          ...
          newKeys.add(`${r.symbol}-${r.tier}-${r.entryPattern}`);
          await new Promise(res => setTimeout(res, 500));
        }
        setSentSignals(newKeys);
      }
    }
  }, [scanning, sentSignals]);
    ...
    <div style={styles.root}>
      <div style={styles.header}>
        <div style={styles.logoRow}>
          ...
          <div style={styles.tagline}>Multi-TF FVG · Live Binance · Telegram Alerts</div>
        </div>
        <div style={styles.headerRight}>
          {lastScan && <span style={styles.lastScan}>Last: {timeAgo(lastScan)}</span>}
          {!scanning && <span style={styles.cdBadge}>⟳ {mins}:{secs.toString().padStart(2,"0")}</span>}
          ...
          <div style={styles.detailHeader}>
            ...
            {selected.entryPattern && (
              <span style={{
                ...
                fontSize:13,
              }}>{selected.entryPattern} pattern</span>
            )}
