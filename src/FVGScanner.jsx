...
// ✅ Calculate ms until next 15M candle close (PHT :00,:15,:30,:45)
...
async function fetchFuturesKlines(symbol, interval, limit = 101) {
  ...
  const closed = raw.slice(0, -1); // exclude unclosed candle
  return closed.map((k) => ({
    time: k[0],
    open: parseFloat(k[1]),
    high: parseFloat(k[2]),
...
// ✅ Check if candle overlaps FVG zone
...
function detectStrictPattern(candles, fvg) {
  ...
  // Rule 1: No doji — minimum 0.1% body
  ...
  // Rule 3: At least ONE of the 3 candles must overlap FVG zone
  const anyInFVG =
...
export default function FVGScanner() {
  ...
  // ✅ Schedule scan at exact 15M candle close
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0a0b0f",
      ...
      <div style={{ padding: "14px 20px", borderTop: "1px solid #1e293b",
        ...
        <span style={{ color: "#f59e0b" }}>🔔 = Browser Alert</span>
